<?php 
  ...
  if ($photo['name']) 
  {
    ...
    $_SESSION['login_user']['profile'] = $dest_path;
  ...
  if($login_user_type == "Staff"){
    ...
    $sql = "UPDATE students SET name=:value1, profile=:value2, dob=:value3, address=:value4, email=:value5 WHERE id=:value6";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':value1', $name);
    $stmt->bindParam(':value2', $dest_path);
    $stmt->bindParam(':value3', $dob);
  ...
  $_SESSION['success_msg']="Your Profile is <b> UPDATED </b> successfully in our database.";
